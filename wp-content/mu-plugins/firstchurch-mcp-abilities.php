...
/* ----------------------------------------------------------------------------
 * MCP resources + prompts. Resources are passive context the client can read
 * (uri in meta, content resolved here at read time); prompts are ready-made
 * workflows whose execute_callback returns { description, messages[] }. Both are
 * abilities flagged meta.mcp.type = resource|prompt + public, so the adapter's
 * default server auto-discovers them into its resources/prompts lists.
 * ------------------------------------------------------------------------- */
