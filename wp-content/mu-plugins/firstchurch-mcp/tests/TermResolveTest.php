 ...
 *
 * Used by the post/category + tag writers to turn human-supplied names or
...
final class TermResolveTest extends TestCase
{
    ...
    {
        $term = fcmcp_test_add_term('category', 'jane-doe', 'Jane Doe');
        $ids  = fcmcp_resolve_terms('category', array('jane-doe'));
        $this->assertSame(array($term->term_id), $ids);
        $this->assertCount(1, $GLOBALS['fcmcp_test']['terms']['category'], 'no new term created');
    }
    ...
    {
        $ids = fcmcp_resolve_terms('post_tag', array('Grace'));
        $this->assertCount(1, $ids);
        $this->assertCount(1, $GLOBALS['fcmcp_test']['terms']['post_tag']);
        // A second resolve of the same value reuses the created term.
        $again = fcmcp_resolve_terms('post_tag', array('Grace'));
        $this->assertSame($ids, $again);
        $this->assertCount(1, $GLOBALS['fcmcp_test']['terms']['post_tag'], 'no duplicate term created');
    }

    public function testSkipsEmptyValuesAndResolvesMixedList(): void
    {
        $jane = fcmcp_test_add_term('category', 'jane-doe', 'Jane Doe');
        $ids  = fcmcp_resolve_terms('category', array('Jane Doe', '', '   ', 'New Person'));
