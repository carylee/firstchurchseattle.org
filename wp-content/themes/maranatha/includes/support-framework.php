 ...
 * @copyright  Copyright (c) 2015 - 2025, ChurchThemes.com
 ...
 * @since 1.0
 */
function maranatha_add_theme_support_framework()
{
	...
	add_theme_support('ctfw-wordpress-version', '5.0'); // 5.0 for block starter content, 4.7 for get_theme_file_uri(), 4.3 at first release.
	...
	add_theme_support('ctfw-after-activation', array(
		...
		'notice'				=> sprintf(__('<b>Next Steps:</b> Please continue reading the <a href="%s" target="_blank">Getting Started</a> guide for the next steps after theme activation.', 'maranatha'), 'https://churchthemes.com/guides/user/getting-started/'),
	...
	add_theme_support('ctfw-edd-license', array(
		'remote_api_url' 			=> 'https://license.churchthemes.link', // Reverse proxy for better reliability (firewalls)
		...
		'options_page_message'		=> sprintf( // shown at top of Theme License page
			__('Please activate your license key to enable one-click theme updates and to get support from <a href="%1$s" target="_blank">ChurchThemes.com</a> for this site. Read <a href="%2$s" target="_blank">License Keys</a> for more information.', 'maranatha'),
			'https://churchthemes.com',
			'https://churchthemes.com/go/license-keys'
		),
		'activation_error_notice'	=> sprintf( // shown when activation fails
			__('<b>License key could not be activated.</b> Read the <a href="%1$s" target="_blank">License Keys</a> guide for help.', 'maranatha'),
			'https://churchthemes.com/go/license-inactive'
		),
		'expired_notice'			=> __('<strong>Theme License Expired:</strong> <a href="%4$s" target="_blank">Renew</a> your <a href="%1$s">Theme License</a> to re-enable updates and support for the <strong>%2$s</strong> theme (expired on <strong>%3$s</strong>). <a href="%5$s" target="_blank">Learn More</a>', 'maranatha'), // optional notice to override default with when license is expired
		'expiring_soon_notice'		=> __('<strong>Theme License Expiring Soon:</strong> <a href="%4$s" target="_blank">Renew</a> your <a href="%1$s">Theme License</a> to continue receiving updates and support for the <strong>%2$s</strong> theme (expires on <strong>%3$s</strong>). <a href="%5$s" target="_blank">Learn More</a>', 'maranatha'), // optional notice to override default with when license expires soon
	));
	...
	add_theme_support('ctfw-replace-wp-text', array(
		'Correo electrónico'	=> __('Email', 'maranatha'), // [Theme Check false positive] Spanish: too long for comment form
		'Adresse de contact'	=> __('Courriel', 'maranatha'), // French: too long for comment form
	));
	...
	add_theme_support('ctfw-ie-unsupported', 8); // version 8 and earlier
	...
	// Page templates must be specified with ctfw_content_types filter. See maranatha_update_content_types().
	...
	add_theme_support('ctfw-loop-after-content-used', 'maranatha_loop_after_content_used');
	...
	add_theme_support(
		...
		array(
			'post'			=> 'maranatha-banner',
			'ctc_sermon'	=> 'maranatha-banner',
			'ctc_event'		=> 'maranatha-banner',
			'ctc_person'	=> 'maranatha-thumb-small',
			'ctc_location'	=> 'maranatha-banner',
			'page'			=> 'maranatha-banner'
		),
		__('Optionally provide an image that is at least %s (it will be cropped/resized). An image exactly this size is ideal.', 'maranatha')
	);
	...
	add_theme_support('ctfw-image-upscaling', array(
		'excluded_sizes' => array(
			'maranatha-section', // big backgrounds; CSS cover/center scales fine
			'maranatha-banner', // big banners; CSS cover/center scales fine
		),
	));
	...
	add_theme_support('ctfw-gallery-thumb-size', array(
		'2' => 'maranatha-rect-large', 		// when 1 to 2 columns used
		'5' => 'maranatha-rect-medium', 	// when 3 to 5 columns used
		'9' => 'maranatha-rect-small',  	// when 6 to 9 columns used
	));
	...
	add_theme_support('ctfw-editor-styles', array(
		...
		'css_function'			=> 'maranatha_head_styles',									// For Classic Editor. Function outputting dynamic CSS in <head> (exclude <style> tag).
		'block_css_function'	=> 'maranatha_block_editor_head_styles',					// For Gutenberg Editor. Function outputting dynamic CSS in admin <head> (exclude <style> tag).
...
add_action('after_setup_theme', 'maranatha_add_theme_support_framework');
 ...
 * @since 1.0
 */
function maranatha_add_theme_support_framework_widgets()
{
	...
	add_theme_support('ctfw-widget-archives', array(
		...
		// See maranatha_archive_widget_types() below for specifying which post types this theme supports archives for.
	...
	add_theme_support('ctfw-widget-highlight', array(
		...
		'field_overrides' => array(
			'image_id' => array(
				'desc' => sprintf(__('Image cropped to %s.', 'maranatha'), ctfw_image_size_dimensions('maranatha-rect-large')),
			),
		),
	));
	...
	add_theme_support('ctfw-widget-section', array(
		...
		'field_overrides' => array(
			...
			'video' => array( // help them know what kind of video to use
				'after_name' => __('(First Section Only)', 'maranatha'), // replace "(Optional)"
				'desc' => sprintf(__('Must use <b>.mp4</b> file. Provide <i>Image</i> above for mobile devices not supporting video. See the <a href="%1$s" target="_blank">Video Guide</a> for important info.', 'maranatha'), 'https://churchthemes.com/go/maranatha-homepage-video'),
			),
			'image_opacity' => array(
				'desc' => __('Lower percentage makes more color show through. Aim for easy to read text.', 'maranatha'),
			),
		),
	));
}

add_action('after_setup_theme', 'maranatha_add_theme_support_framework_widgets');
 ...
 */
function maranatha_archive_widget_types($options)
{
	...
	return apply_filters('maranatha_archive_widget_types', $options);
}

add_filter('ctfw_archives_widget_post_type_options', 'maranatha_archive_widget_types');
